...
public class TpahereCommand implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
		...
		if (!sender.hasPermission(Permissions.TPAHERE)) {
			sender.sendMessage(Strings.NO_PERMISSION);
			return true;
		}

		if (args.length == 0) {
			sender.sendMessage(Strings.USAGE + "/tpahere <player>");
			return true;
		}
		...
		BaseComponent[] targetComponent = new ComponentBuilder()
				.append(ChatColor.GOLD + player.getName() + ChatColor.GRAY + " has requested you to teleport to them.\n")
		...
		TeleportManager.addRequest(player.getUniqueId(), target.getUniqueId());
	...
	public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
		...
		if (!sender.hasPermission(Permissions.TPAHERE)) return arguments;
