...
public class Sethome implements CommandExecutor {
	...
	public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
		...
		if (HomeManager.getHomeCount(player.getUniqueId()) == Utilities.config.home.maxHomes) {
		...
		DatabaseOperations.addHome(player.getUniqueId(), name, location, () -> {
			HomeManager.addHome(player.getUniqueId(), new Home(name, location.getWorld().getName(), location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch()));
